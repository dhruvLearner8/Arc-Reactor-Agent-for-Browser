...
class ExecutionContextManager:
    ...
    def _has_executable_code(self, output):
        ...
        if not isinstance(output, dict):
            return False
        if output.get("_already_executed"):
            return False
    ...
    async def _auto_execute_code(self, step_id, output, extra_vars=None):
        ...
        for code_key, code in code_to_execute.items():
            try:
                ...
                # 4. Inject transient per-step variables from previous iterations
                # so generated code can reference intermediate names like found_urls_Txxx.
                normalized_globals_schema = {
                    k: self._ensure_parsed_value(v) for k, v in globals_schema.items()
                }
                if extra_vars and isinstance(extra_vars, dict):
                    for var_name, var_value in extra_vars.items():
                        parsed_value = self._ensure_parsed_value(var_value)
                        globals_injection += f'{var_name} = {repr(parsed_value)}\n'
                        normalized_globals_schema[var_name] = parsed_value
                
                # Ensure generated code that scans globals_schema sees normalized values.
                globals_injection += f'globals_schema = {repr(normalized_globals_schema)}\n'
        ...
        return {"status": "error", "error": last_error or "All code variants failed"}
    ...
    def _record_data_quality(self, key, value):
        """Track whether extracted data is meaningful vs placeholder/empty."""
        flags = self.plan_graph.graph.setdefault("data_quality", {})
        is_empty = (
            value is None
            or value == ""
            or value == []
            ...
            for write_key in writes:
                ...
                if execution_result and execution_result.get("status") == "success":
                    ...
                    if write_key in result_data:
                        globals_schema[write_key] = self._ensure_parsed_value(result_data[write_key])
                        ...
                        globals_schema[write_key] = self._ensure_parsed_value(value)
                ...
                if not extracted and output and isinstance(output, dict):
                    ...
                    # 🎉 NEW: Check for 'final_answer' as fallback (Summarizer often uses this)
                        ...
                        globals_schema[write_key] = self._ensure_parsed_value(output["final_answer"])
                        print(f"✅ Extracted {write_key} = [Final Answer] (mapped from 'final_answer')")
                        extracted = True
                    elif len(writes) == 1:
                        metadata_keys = {
                            "call_self", "code_variants", "next_instruction",
                            "thought", "cost", "input_tokens", "output_tokens", "total_tokens"
                        }
                        candidate_keys = [
                            k for k in output.keys()
                            if k not in metadata_keys and isinstance(k, str)
                        ]
                        preferred_key = next(
                            (
                                k for k in candidate_keys
                                if any(token in k.lower() for token in ["formatted_report", "final_report", "summary", "report", "answer"])
                            ),
                            None
                        )
                        if preferred_key:
                            preferred_value = self._ensure_parsed_value(output[preferred_key])
                            if (
                                preferred_key not in {"summary_unavailable", "missing_inputs", "reason"}
                                and not isinstance(preferred_value, bool)
                                and preferred_value not in (None, "", [], {})
                            ):
                                globals_schema[write_key] = preferred_value
                                print(f"✅ Extracted {write_key} = {output[preferred_key]} (fallback from {preferred_key})")
                                extracted = True
                        elif len(candidate_keys) == 1:
                            fallback_key = candidate_keys[0]
                            fallback_value = self._ensure_parsed_value(output[fallback_key])
                            if (
                                fallback_key not in {"summary_unavailable", "missing_inputs", "reason"}
                                and not isinstance(fallback_value, bool)
                                and fallback_value not in (None, "", [], {})
                            ):
                                globals_schema[write_key] = fallback_value
                                print(f"✅ Extracted {write_key} = {output[fallback_key]} (fallback from {fallback_key})")
                                extracted = True
                
                # Strategy 3: Emergency fallback - try to find any matching data
                if not extracted:
                    print(f"⚠️  Could not extract {write_key}")
                    # Avoid empty placeholders because downstream read-gates treat
                    # []/{} as missing and can short-circuit later nodes.
                    globals_schema[write_key] = self._ensure_parsed_value({
                        "status": "insufficient_data",
                        "reason": f"No extractable value found for required key `{write_key}`."
                    })
                self._record_data_quality(write_key, globals_schema.get(write_key))
    ...
    def get_inputs(self, reads):
        """Get input data from graph globals_schema with robust type normalization."""
        inputs = {}
        globals_schema = self.plan_graph.graph['globals_schema']

        for read_key in reads:
            if read_key in globals_schema:
                raw_val = globals_schema[read_key]
                inputs[read_key] = self._ensure_parsed_value(raw_val)
            else:
    ...
    def get_execution_summary(self):
        ...
        return {
            ...
            "data_quality": self.plan_graph.graph.get("data_quality", {})
        }
    ...
    def _auto_save(self):
        ...
        if self.debug_mode:
            return
        # Production mode: keep chat/run persistence off local disk.
        if (os.getenv("SAVE_LOCAL_SESSIONS") or "1").strip().lower() in {"0", "false", "off", "no"}:
            return
        try:
            self._save_session()
        except Exception as e:
    ...
    def _ensure_parsed_value(self, value, _depth=0, _max_depth=5):
        """
        Normalize stringified Python/JSON objects into real Python types.
        Examples:
        "['a','b']" -> ['a', 'b']
        "{'x': 1}" -> {'x': 1}
        '"[1,2,3]"' -> [1, 2, 3] (double-encoded case)
        """
        if _depth > _max_depth:
            return value

        # Recurse containers first
        if isinstance(value, dict):
            return {
                self._ensure_parsed_value(k, _depth + 1, _max_depth): self._ensure_parsed_value(v, _depth + 1, _max_depth)
                for k, v in value.items()
            }
        if isinstance(value, list):
            return [self._ensure_parsed_value(v, _depth + 1, _max_depth) for v in value]
        if isinstance(value, tuple):
            return tuple(self._ensure_parsed_value(v, _depth + 1, _max_depth) for v in value)

        # Only strings may need parsing
        ...
        # Fast gate: only try parsing object-like strings
        looks_structured = (
        ...
        for parser in (json.loads, ast.literal_eval):
            try:
                parsed = parser(s)
                # Recurse again in case of double-encoded content
                return self._ensure_parsed_value(parsed, _depth + 1, _max_depth)
            except Exception:
                pass
