...
class AgentRunner:
    ...
    async def run_agent(self, agent_type: str, input_data: dict, image_path: Optional[str] = None) -> dict:
        ...
        try:
            ...
            # 6. Parse JSON response dynamically (with one strict recovery retry)
            try:
                output = parse_llm_json(response)
            except JsonParsingError:
                writes_hint = input_data.get("writes", [])
                repair_prompt = (
                    f"{full_prompt}\n\n"
                    "Your previous response was not valid JSON.\n"
                    f"Return ONLY a valid JSON object. Use exact keys from writes: {writes_hint}.\n"
                    "No markdown. No prose. No code fences."
                )
                response = await model_manager.generate_text(repair_prompt)
                output = parse_llm_json(response)
