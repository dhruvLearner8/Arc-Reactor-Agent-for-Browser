    ...
    if context:
        ...
        if "final_outputs" in summary and summary["final_outputs"]:
            ...
            if isinstance(final_outputs, dict):
                # Prefer explicit markdown-like keys from formatter outputs.
                for key, value in final_outputs.items():
                    if isinstance(value, str) and "markdown" in key.lower():
                        return value

                # Fallback: return the longest textual output.
                text_values = [v for v in final_outputs.values() if isinstance(v, str)]
                if text_values:
